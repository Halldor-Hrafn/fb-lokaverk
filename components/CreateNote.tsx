...
const CreateNote: React.FC<NoteProps> = ({ initial }) => {
  ...
  return (
    <div className="w-full flex flex-col gap-6 items-center">
      <form action={createNote} className="bg-inherit shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <div className="mb-6">
          <label className="block text-inherit text-sm font-bold mb-2" htmlFor="note">
            Note:
          </label>
          <textarea className="bg-inherit text-inherit shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="content" id="content" cols={30} rows={10} />
        </div>
